...
页面还提供 512/256 射线精度、对手预测模型、实时 MPC telemetry 和参数面板。手机端支持
横屏、全屏、左侧移动轮盘和右侧开火。游戏物理以 25 Hz（每帧 40 ms）运行，页面按显示器
刷新率绘制并在相邻物理状态之间插值。
...
其中 $B$ 是弹匣容量，$b$ 是剩余可发射子弹数。预计命中的奖励还会根据飞行时间和
...
$$
a^*=\underset{a\in\mathcal A}{\operatorname{arg\,max}}\;S(a).
$$
