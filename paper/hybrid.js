...
export class HybridPolicy {
  ...
  constructor(manifest, weights) {
    if (manifest.schema !== 26 || manifest.observation !== OBS_DIM || manifest.actions !== 18) {
      throw new Error("Hybrid model does not match engine schema 26 / 1038 / 18");
    }
  ...
  logits(observation, mask, dodge) {
    ...
    const scalarInput = new Float32Array(98);
