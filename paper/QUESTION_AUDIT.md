...
> 2026-09-21：页面的问题清单重排为 8 条，新增「超参数是怎么定的」「PPO 的训练循环到底在做什么」「训练里碰到了什么，怎么证明是改动有效而不是训练步数上来了」「为什么 actor 捷径没用，entropy 在这里说明了什么」四条；删除原 01（强度归因）、06（抖动是 loss 问题）、10（能否被针对）、11（v16 与 v17b 并列）。原 07（PPO 正确性）并入新 02，原 09（调度与血统污染）并入新 03，原 04（六条捷径）并入新 05。本文档下列各节按旧编号书写，对照关系以本条为准。
