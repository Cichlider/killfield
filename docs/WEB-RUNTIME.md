...
1. 浏览器 `keydown` 记录按键与开始时间，`keyup` 把真实按下毫秒数累计进队列；窗口失焦
   会清空全部按键，避免丢失 `keyup` 后永久卡键。
2. 每个 `40ms` 物理 tick 对队列采样，把前进、后退、左右转换成 `0..1` 连续强度；开火
   转为独立布尔值。
...
这种时长积分能让短按小于一帧时仍产生相称动作，但也会产生粘滞感：MPC 在主线程同步规划
期间，按下时间仍继续累计；如果渲染回调迟到，固定步进循环会补算最多 `250ms`，队列中的
输入时长也会在这些补帧中逐帧消费。因此用户已经松键后，仍可能看到先前积压的输入尾巴。
除此之外，25Hz 采样天然还有最多约 `40ms` 的释放延迟。当前这里只记录行为，没有更改键盘
输入语义。
