...
The first destruction layer (`destroy.wav`, originally `explosion3.wav`) is
also the wall-impact effect for the frag bomb. The current duel has no frag
bomb, so that second trigger is not active yet.
