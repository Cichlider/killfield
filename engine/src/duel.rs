...
use crate::duel_obs::SeatHistory;
...
/// A mutual kill still means this agent died, so it carries a small survival
/// cost. It remains far better than being beaten, while no longer making a
/// reckless trade look harmless. The pressure to engage comes from the clock:
/// refusing to fight runs into `REWARD_DRAW`, which costs as much as losing.
pub const REWARD_DOUBLE_DEATH: f64 = -0.1;
...
/// `S < 0.6` with the current `-0.1` trade. `0.25` leaves ample daylight on
/// both comparisons and also keeps the worst trade above the best loss.
...
pub struct DuelState {
    ...
    pub shots_fired: u32,
    pub hits: u32,
    /// Recent actions and the running change count, per seat. A frozen
    /// opponent is the same network in the other chair, so it needs its own
    /// copy: the channels mean "what *I* did", whoever is reading.
    own_history: SeatHistory,
    opponent_history: SeatHistory,
}
...
impl DuelState {
    pub fn new(seed: u32, opponent: Opponent, game: &Game) -> Self {
        ...
        Self {
            ...
            shots_fired: 0,
            hits: 0,
            own_history: SeatHistory::default(),
            opponent_history: SeatHistory::default(),
        }
    }
    ...
    pub fn record_action(&mut self, action: u16) {
        self.own_history.record(action);
    }

    /// How many frames chose a different action than the frame before. This is
    /// what `style_bonus` grades, and it counts the policy's seat only.
    pub fn action_changes(&self) -> u32 {
        self.own_history.changes
    }

    /// Each seat's own recent-action history, stamped with the round's frame
    /// count so the encoder can turn the change count into a rate.
    pub fn own_history(&self) -> SeatHistory {
        SeatHistory { frames: self.frames, ..self.own_history }
    }

    pub fn opponent_history(&self) -> SeatHistory {
        SeatHistory { frames: self.frames, ..self.opponent_history }
    }
    ...
    pub fn before_step_with(&mut self, game: &mut Game, opponent_action: Option<u16>) {
        ...
        if self.opponent.is_external() {
            if let Some(action) = opponent_action {
                self.opponent_history.record(action);
                if game.tanks[1].alive && !game.frozen {
    ...
    pub fn opponent_last_action(&self) -> Option<u16> {
        self.opponent_history.actions[0]
    }
}
...
mod tests {
    ...
    fn the_scale_is_ordered_win_trade_loss_stall() {
        ...
        assert!(
            ...
            "a trade includes our death and must carry a survival cost"
        );
        ...
        // below the break-even point. Guard a broad sane band rather than the
        // exact number; the survival penalty intentionally puts a fresh
        // round's threshold at 45%.
